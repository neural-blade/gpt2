...
void resid(float *a, const float *b, uint64_t len);

void gelu_actv(float *x, uint64_t len);

void out_proj(const float *in, const float *weight, float *out,
	      uint32_t hidden_dim, uint32_t out_dim);

void greedy_decode(const float *v, uint64_t len, uint32_t *out);
